...
from pivots_calc import find_pivot, pivothigh, pivotlow
...
def back_test():
    ...
    df["tinyic"] = df.apply(tiny_ic, axis=1)
    ...
    df2 = get_second_df(c, d, df)
    df3 = get_third_df(df, df2)
    df4 = get_fourth_diff(a, b, df)
    df = get_fifth_df(df, df3, df4)
    ...
    for i in df.index:
        ...
        if(look_for=='SPH'):
            ...
            if(str(df['SPH'][i]) != 'nan'):       
                ...
                print('111')
                # Implement check for cases when SPH and SPL are in same row
                if(str(df['SPL'][i]) != 'nan'):       
    ...
    #####
    """ Need to add a function to plot the pivots if and when necessary """
    #####

    """ Trend calculation  """

    w = df[df['newSPL'].notnull()].index.tolist() # Index for plotting 
    a = list(df.newSPL.values)
    #.dropna(inplace=True)
...
def get_second_df(c, d, df):
    df2 = pd.DataFrame(list(zip(c, d)), columns=['SPL','id11'])
...
def get_third_df(df, df2):
    df3 = pd.merge(left = df, on = "Date_Time", right = df2[['Date_Time','SPL']], how = 'outer')
    df3.head()
    return df3

def get_fourth_diff(a, b, df):
    df4 = pd.DataFrame(list(zip(a,b)), columns=['SPH','id11'])
